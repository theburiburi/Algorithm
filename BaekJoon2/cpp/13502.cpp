#include <iostream>
#include <string>
#include <vector> // fstream 대신 vector 추가
...
void dfs(int y, int x, Trie *node, string cur_word)
{
    if (node->is_end)
    {
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (node->child[next - 'A'])
        {
            visited[ny][nx] = true;
            dfs(ny, nx, node->child[next - 'A'], cur_word + next);
            visited[ny][nx] = false;
        }
    }
}
